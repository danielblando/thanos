...
func enrichTargetsWithExtLabels(targets *targetspb.TargetDiscovery, extLset labels.Labels) {
	for i, target := range targets.ActiveTargets {
		target.SetDiscoveredLabels(labelpb.ExtendSortedLabels(target.DiscoveredLabels.PromLabels(), labelpb.LabelsToToPromLabelSets(extLset)))
		target.SetLabels(labelpb.ExtendSortedLabels(target.Labels.PromLabels(), labelpb.LabelsToToPromLabelSets(extLset)))
